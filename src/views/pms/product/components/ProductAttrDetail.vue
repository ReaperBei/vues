  ...
  <div style="margin-top: 50px">
    <el-form :model="value" ref="productAttrForm" label-width="120px" style="width: 720px" size="small">
      ...
      <el-form-item label="商品规格：">
        ...
        <el-table style="width: 100%;margin-top: 20px" :data="value.skuStockList" border>
          ...
          <el-table-column
            ...
            align="center">
            <template slot-scope="scope">
              <el-input v-model="scope.row.sendPoint"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            label="操作"
      ...
      <el-form-item label="属性图片：" v-if="hasAttrPic">
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductAttrPics" :key="index">
            <span>{{item.name}}:</span>
            <upload v-if="true" :list="isEdit?[item.pic]:[]" :limit="1" :type="4002" v-model="item.pic" :index="index" @changeImageList="getChangeImage"/>
          </div>
        </el-card>
      </el-form-item>
      ...
      <el-form-item label="商品相册：">
              <upload v-if="true" :list="selectProductPics"  @getList="getChangeImageArry" />
      </el-form-item>
      <el-form-item label="规格参数：">
        <el-tabs v-model="activeHtmlName" type="card">
          <el-tab-pane label="电脑端详情" name="pc">
             <vue-editor useCustomImageHandler  @image-added="handleImageAdded" class="editor-box" :editorToolbar="customToolbar" v-model="value.detailHtml" > </vue-editor>
          </el-tab-pane>
          <el-tab-pane label="移动端详情" name="mobile">
                <vue-editor useCustomImageHandler  @image-added="handleImageAdded" :editorToolbar="customToolbar" v-model="value.detailMobileHtml" > </vue-editor>
          </el-tab-pane>
        </el-tabs>
      </el-form-item>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      selectProductPics:{
        get:function () {
          ...
          for(let i=0;i<=albumPics.length-1;i++){
    ...
    mounted(){
      var he =document.getElementsByClassName('ql-container')
      for(var i = 0;i<he.length;i++){
        he[i].style='height: 500px;overflow-y: auto;overflow-x: hidden;'
      }
    },
    methods: {
      ...
      getChangeImage(item){
        this.selectProductAttrPics[item.index].pic= item.pic
      },
      getChangeImageArry(urls){
          this.selectProductPics = urls
      },
      //获取商品相关属性的图片
