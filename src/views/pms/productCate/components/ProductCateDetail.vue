  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="productCate"
      ...
      <el-form-item label="分类图标：">
        <SingleUpload v-if="dialogVisible" :list="isEdit?[productCate.icon]:[]" :limit="1" :type="4002" v-model="productCate.icon" />
      </el-form-item>
      ...
      if (this.isEdit) {
        getProductCate(this.$route.query.id).then(response => {
          ...
          if(this.productCate.parentId=='0'){
            this.productCate.parentId = '无上级分类'
          }
        });
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              if (this.isEdit) {
                if(typeof(this.productCate.icon)!=='string'){
                  this.productCate.icon = this.productCate.icon[0]
                }
