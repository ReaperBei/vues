<template> 
  <el-card class="form-container" shadow="never">
    <el-form :model="coupon"
      ...
      <el-form-item label="优惠券生效时间：" prop="enableTime">
        <el-date-picker
          type="datetime"
          :picker-options="pickerOptions"
          placeholder="选择日期"
          v-model="coupon.enableTime"></el-date-picker>
      </el-form-item>
  ...
  export default {
    ...
    data() {
      ...
      var validate = (rule, value, callback) => {
        ...
        if (_this.coupon.startTime>=value) {
          callback(new Error('结束日期要大于开始日期'))
        } else {
          callback()
        }
      };
      var valiformdate = (rule, value, callback) => {
         if (!value) {
          return callback(new Error('请选择生效日期'))
        }
        if (_this.coupon.startTime>value||_this.coupon.endTime<value) {
          callback(new Error('生效日期要大于等于开始日期小于等于结束日期'))
        } else {
           callback()
        }
      };
      var formData = (rule, value, callback) => {
        const reg = /^[0-9]+(.?[0-9]{1,2})?$/
         if (!value) {
          return callback(new Error('请选择面额'))
        }
        if (!reg.test(value)) {
          callback(new Error('面值只能是数值，0.01-10000，限2位小数'))
        } else {
           callback()
        }
      };
      var valiminPoint = (rule, value, callback) => {
        const reg = /^[1-9]\d*$/
         if (!value) {
          return callback(new Error('请选择内容'))
        }
        if (!reg.test(value)) {
          callback(new Error('请输入数字且只能是整数'))
        } else {
           callback()
        }
      };
      return {
        ...
        rules: {
          name: [
           {type: 'number',required: true, validator:regular.FormValidate.Form().validateCharacter, trigger: 'blur'}
          ],
          startTime:[
          ...
          enableTime:[
            {required: true, validator:valiformdate, trigger: ['blur','change']},
          ],
          publishCount: [
            {type: 'number',required: true, validator:regular.FormValidate.Form().valiminPoint, trigger: 'blur'}
          ],
          amount: [
            {type: 'number',required: true,validator: formData,trigger: 'blur'}
          ],
          perLimit:[
            {type: 'number',required: true,validator:regular.FormValidate.Form().valiminPoint,trigger: 'blur'}
          ],
    ...
    methods:{
      ...
      handelData(){
        let enableTime = this.formatDate(this.coupon.enableTime)
        let startTime = this.formatDate(this.coupon.startTime)
        let endTime = this.formatDate(this.coupon.endTime)
        Object.assign(this.coupon,{enableTime:enableTime,endTime:endTime,startTime:startTime})
      },
      trimLR(vaules) {
        if(this.coupon.name==vaules){
          this.coupon.name = vaules.replace(/^\s+|\s+$/gm,'')
        }
      },
      onSubmit(formName) {
